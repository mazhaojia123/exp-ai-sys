...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    examples = _____________________________(input_file=input_file,
                                            is_training=not evaluate,
                                            version_2_with_negative=args.version_2_with_negative)
    #TODO: 调用utils.utils_squad函数将SQuAD测例转换为特征表示
    features = _____________________________(examples=examples,
                                            tokenizer=tokenizer,
    ...
    all_input_mask =  _____________________________
    #TODO: 从 features 列表中提取 segment_ids 属性，形成一个列表
    all_segment_ids = _____________________________
    #TODO: 从 features 列表中提取 cls_index 属性，形成一个列表
    all_cls_index =   _____________________________
    #TODO: 从 features 列表中提取 p_mask 属性，形成一个列表
    all_p_mask =      _____________________________
    
    #TODO: 将之前创建的五个列表转换为 NumPy 数组
    all_input_ids =   _____________________________
    all_input_mask =  _____________________________
    all_segment_ids = _____________________________ 
    all_cls_index =   _____________________________
    all_p_mask =      _____________________________ 
...
def evaluate(args, tokenizer, prefix=""):
    # TODO: 调用数据加载模块加载数据集
    dataset, examples, features = _____________________________________________________________________________
    ...
    eval_dataloader = _______________________________________(dataset, len(features), batch_size=args.eval_batch_size)
    ...
    example_bert_pre_weights, example_encoder_weights, example_bert_post_weights = ___________________________________
    logger.info("Bert: loading weights...")
    #TODO: 调用PyBert中的函数以加载权重到模型
    ___________________________________(example_bert_pre_weights, example_encoder_weights, example_bert_post_weights)
    ...
    for sample in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        logits, pooler_out, hwtime, fwtime  = _____________________(_________, _________,_________, "/workspace/cambricon_demo/Bert")
        #TODO: 将模型输出的 logits 转换为浮点数
        logits=_______________________________###################################here
...
def main():
    #TODO: # 创建命令行参数解析器
    parser = ______________________________________

    ## Required parameters
    #TODO：给一个解析器添加程序参数信息
    _______________________("--predict_file", default=None, type=str, required=True,
                        help="SQuAD json for predictions. E.g., dev-v1.1.json or test-v1.1.json")
    _______________________("--output_dir", default=None, type=str, required=True,
    ...
    #TODO: 解析命令行参数
    args = ______________________________________
