...
lib = _______________________________________
...
i32_p = _______________________________________
type_map = {
    numpy.float16: u16_p,
    half: u16_p,
    int32: i32_p,
...
class PyBert:
    ...
    def allocate(self, dictionary_len: int, type_dictionary_len: int, encoder_layers: int, eos_id: int, bos_id: int):
        ...
        max_batch_size      = _______________________ 
        max_seq_len         = _______________________ 
        head_size           = _______________________ 
        head_num            = _______________________
        ffn_hidden          = _______________________
        hidden_act          = _______________________
        calc_type           = 1 
        filter_type         = 1 

        #TODO: 调用libst.so的bertAllocate 函数以及上述一系列参数，完成bert 模型在 MLU内存上的分配，并将 bertAllocate 返回的分配地址传递给 self.ptr
        self.ptr = _________________________________
    ...
    def load_weight(
        ...
        encoder_self_attn_layernormal_scale_layers = extract(encoder_layers_weights, "self_attn", "layernormal_scale")
    ...
    def forward(
        ...
        seq_output        = to_ptr(seq_output)
        pooler_output     = to_ptr(pool_output)
        hwtime            = ctypes.byref(hwtime)
        fwtime            = ctypes.byref(fwtime)
